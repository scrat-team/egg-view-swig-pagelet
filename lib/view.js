...
class PageletView {
  ...
  * renderPage(name, locals, options = {}) {
    ...
    const fakePath = path.join(this.app.swigPagelet.viewRoot, normalName.replace(/\//g, '_') + '.' + this.ext);
    const source = `{% extends '${layout}' %}{% block content %}{% require $id='page/${normalName}' %}{% endblock %}`;
    const tplFn = this.app.swigPagelet.compile(source, { filename: fakePath });
    return tplFn(locals);
  }
}
